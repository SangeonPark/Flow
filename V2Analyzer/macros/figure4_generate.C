	...
	for (int m = 0; m < 10; ++m)
	{
		...
		for (Int_t i = 0; i < NAchBins; i++){
			for(Int_t j = 0 ; j < 4; j++){
				...
				c2_tot_case1[i][j][0] = (TH1D*)f->Get(Form("demo_n9/c2tot_%d_%d_cos_case1",i,j));
				c2_tot_case1[i][j][1] = (TH1D*)f->Get(Form("demo_n9/c2tot_%d_%d_sin_case1",i,j));
				c2_pos_case1[i][j][0] = (TH1D*)f->Get(Form("demo_n9/c2pos_%d_%d_cos_case1",i,j));
				c2_pos_case1[i][j][1] = (TH1D*)f->Get(Form("demo_n9/c2pos_%d_%d_sin_case1",i,j));
				c2_neg_case1[i][j][0] = (TH1D*)f->Get(Form("demo_n9/c2neg_%d_%d_cos_case1",i,j));
				c2_neg_case1[i][j][1] = (TH1D*)f->Get(Form("demo_n9/c2neg_%d_%d_sin_case1",i,j));

				c2_tot_case2[i][j][0] = (TH1D*)f->Get(Form("demo_n9/c2tot_%d_%d_cos_case2",i,j));
				c2_tot_case2[i][j][1] = (TH1D*)f->Get(Form("demo_n9/c2tot_%d_%d_sin_case2",i,j));
				c2_pos_case2[i][j][0] = (TH1D*)f->Get(Form("demo_n9/c2pos_%d_%d_cos_case2",i,j));
				c2_pos_case2[i][j][1] = (TH1D*)f->Get(Form("demo_n9/c2pos_%d_%d_sin_case2",i,j));
				c2_neg_case2[i][j][0] = (TH1D*)f->Get(Form("demo_n9/c2neg_%d_%d_cos_case2",i,j));
				c2_neg_case2[i][j][1] = (TH1D*)f->Get(Form("demo_n9/c2neg_%d_%d_sin_case2",i,j));
...
//negative
	...
	for (Int_t i = 0; i < NAchBins; i++){
		...
		ach_hist[i] = (TH1D*)f->Get(Form("demo_n9/ach_%d",i+1));
...
//error bars
	...
	TFile *rebinned = new TFile("~/Summer2016/root_forgraphs/figure4_notcorrected.root","RECREATE");
	...
	TGraphErrors *gr_diff_v2 = new TGraphErrors(NAchBins,x,v2_diff,NULL,err_diff);
	TF1* fit1_v2 = new TF1("f1", "[0]+x*[1]", -0.13, 0.13);

	gr_diff_v2->RemovePoint(0);
	gr_diff_v2->RemovePoint(5);

	gr_diff_v2->Write();

	fit1_v2->SetLineColor(kRed);
	fit1_v2->SetLineStyle(2);
	gr_diff_v2->Fit(fit1_v2,"RN0");

	fit1_v2->Write();

	gr_diff_v2->Draw("PSame");
	fit1_v2->Draw("Same");
