...
//negative
	...
	TFile *rebinned = new TFile("~/Summer2016/root_forgraphs/figure_selfcorrelation_ver2.root","RECREATE");
	gr_pos_poseta->Write();
	gr_neg_poseta->Write();
...
*/
	...
	base->GetXaxis()->SetTitle("Observed A_{ch}(group 1)");
	base->GetYaxis()->SetTitle("v_{2}{2}(group 2)");
	...
	base2->GetXaxis()->SetTitle("Observed A_{ch}(group 1)");
...
/*	gr_diff_poseta->GetYaxis()->SetRangeUser(-0.004,0.004);
	...
	c1->Print("~/Summer2016/selfcorr_SP_ver2.pdf");
	c1->Print("~/Summer2016/selfcorr_SP_ver2.gif");
