...
void appletoapple(){
	...
	double alice_v2_pos[10];
	double alice_v2_neg[10];
	double alice_v2_diff[10];
	double alice_err_pos[10];
	double alice_err_neg[10];
	double alice_err_diff[10];
	double alice_x[10];



	double star_v2_pos[5]= { 3.24, 3.19, 3.18, 3.17, 3.14 };
	double star_v2_neg[5]={ 3.18, 3.19, 3.22, 3.23, 3.29 };
	double star_v2_diff[5]= { -0.0564, -0.00115, 0.039, 0.065, 0.146 };
	double star_err_pos[5] = { 0.0103, 0.00962, 0.00935, 0.00936, 0.0092 };
	double star_err_neg[5]={ 0.00954, 0.00933, 0.0093, 0.00956, 0.0098 };
	double star_err_diff[5];
	double star_x[5]= { -0.028, -0.00841, 0.00325, 0.0149, 0.0345 };
	//double star_x[5]= { -0.068, -0.021, 0.007, 0.035, 0.082 };
	for(Int_t i=0; i<5; i++){


		variance_pos = star_err_pos[i]*star_err_pos[i];


		variance_neg = star_err_neg[i]*star_err_neg[i];



		

		sum = star_v2_pos[i] + star_v2_neg[i];

		star_v2_diff[i] = (star_v2_neg[i] - star_v2_pos[i])/(star_v2_neg[i] + star_v2_pos[i]);


		variance_diff = (4*star_v2_neg[i]*star_v2_neg[i]*variance_pos)/(sum*sum*sum*sum)+(4*star_v2_pos[i]*star_v2_pos[i]*variance_neg)/(sum*sum*sum*sum);


	//error bars

		star_err_diff[i] = sqrt(variance_diff);


	}
	TGraphErrors *star_diff = new TGraphErrors(5,star_x,star_v2_diff,NULL,star_err_diff);
	star_diff -> SetMarkerStyle(28);
	star_diff -> SetMarkerColor(kBlack);

	TF1* fit_star = new TF1("f1", "[0]+x*[1]", -0.13, 0.13);

	fit_star->SetLineColor(kBlue);
	fit_star->SetLineStyle(2);
	star_diff->Fit(fit_star,"RN0");
...
//	TFile *rebinned = new TFile("~/Summer2016/root_forgraphs/figure2_0.root","RECREATE");
 ...
 //   TCanvas* c2 = new TCanvas("c2","c2");
	...
	TLatex* text4 = makeLatex(Form("Intercept(STAR) : %f #pm %f",fit_star->GetParameter(0),fit_star->GetParError(0)),0.45,0.40) ;
	TLatex* text3 = makeLatex(Form("slope(STAR) : %.4f #pm %.4f",fit_star->GetParameter(1),fit_star->GetParError(1)),0.45,0.35) ;
	...
	leg2->AddEntry(fit_star, "star fit","l");
	leg2->AddEntry(star_diff , "star data","p");
	leg2->DrawClone("Same");

	//SaveCanvas(c3,"pics",Form("appletoapple_withoutachcorrection"));
